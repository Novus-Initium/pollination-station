...
const setupEthereumWatcher = async () => {
  try {
    ...
    contract.on('DAOCreated', async (daoAddress, title, event) => {
      ...
      try {
        const response = await fetch(`${process.env.SUPABASE_URL}/functions/v1/dao-manager`, {
          method: 'POST',
          headers: {
    ...
    contract.on('NeedCreated', async (needId, daoAddress, description, event) => {
      ...
      try {
        ...
        const daoResponse = await fetch(`${process.env.SUPABASE_URL}/functions/v1/dao-manager?public_address=${daoAddress}`, {
          headers: {
            'Authorization': `Bearer ${process.env.SUPABASE_ANON_KEY}`
          }
        });
        
        if (!daoResponse.ok) throw new Error('Failed to fetch DAO');
        const daoData = await daoResponse.json();
        
        // Create the need
        const response = await fetch(`${process.env.SUPABASE_URL}/functions/v1/need-manager`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${process.env.SUPABASE_ANON_KEY}`,
            'Content-Type': 'application/json'
